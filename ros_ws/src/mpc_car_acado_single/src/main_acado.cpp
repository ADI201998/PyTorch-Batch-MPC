...
class MinimalPublisher : public rclcpp::Node
{
    ...
    {   
		if(Gotit.maxCoeff() == 0)
		{
			...
			RCLCPP_INFO(this->get_logger(),"Got feedback %.2f", lc);
			Gotit = 1;
...
void MinimalPublisher :: optimize(int info)
{
	...
	{	
		...
		acadoVariables.WN[(NYN+1)*0] = 1*1e9;		// x terminal
		acadoVariables.WN[(NYN+1)*1] = 1*1e9;		// y terminal
		acadoVariables.WN[(NYN+1)*2] = 1e9;		// theta terminal
		acadoVariables.WN[(NYN+1)*3] = 1e6;	

		/*for (int i = 0; i < N; i++)
		{
			acadoVariables.lbAValues[i*2+0] = -(2.75 +3.5*(lc-1));       //y

			acadoVariables.ubAValues[i*2+0] = (2.75 +3.5*(lc-1));       //y
		}*/	
