...
class Lane_optimizer_cubic():
    ...
    def update_vals(self, X, Y, limits, scale):
        ...
        self.params = self.fit_curve()
        if len(self.limits)%2 == 0:
            self.centerline_params = [self.params_old[0], self.params_old[1], self.params[2], self.scale + self.params[3]*(len(self.limits)/2-0.5)]
        else:
            self.centerline_params = [self.params_old[0], self.params_old[1], self.params[2], self.scale + self.params[3]*(len(self.limits)/2-0.5)]
...
class MinimalSubscriber(Node):
    ...
    def projection_linear(self):
        ...
        for l in ind_min:
            ...
            for i in X_input[l]:
                ...
                xpt.append(i[0])
                ypt.append(-1*i[1])
            save_points.append([xpt, ypt])
            
        print("SAVE  _POINTS = ", save_points)
        print(save_points)
        np.save("Lane-Points/lidar2/"+str(self.loop)+".npy", save_points)
        ##print(s2-s1)
    ...
    def projection_quad(self):
        ...
        s2 = time.time()
        ##print(s2-s1)
        X_input[:,1] = -1*X_input[:,1]
        X_input = X_input[:, :2]
        np.save("Lane-Points/ground_truth2/"+str(self.loop)+".npy", X_input)
    ...
    def projection_cubic(self):
        ...
        #print(np.arctan2(np.diff(y_centerline), np.diff(xdata_plot))*180/np.pi)
            ...
            if self.Gotit or self.flag:
                ...
                if len(self.steps)>0:
                    ...
                    if self.flag == 0:
                        ...
                        self.ax2.text(self.lower_lim+170, 10, 'C= %s'%(round(self.L.centerline_params[1],3)), fontsize=20)
                ...
                self.csm.vehicle.set_transform(random.choice(self.csm.m.get_waypoint(self.csm.vehicle.get_location(), project_to_road=True).next(10)).transform)
                self.csm.simulate()
                ...
                xp,yp,_ = self.projection_quad()#self.projection_new()
                self.x_prev.append([xp, 0, psip])
