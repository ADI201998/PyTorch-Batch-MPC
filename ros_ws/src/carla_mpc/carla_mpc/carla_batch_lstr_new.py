...
class Lane_optimizer_linear():
    ...
    def update_vals(self, X, Y, limits, scale):
        ...
        if len(self.limits)%2 == 0:
            self.centerline_params = [self.params[0], self.scale + self.params[1]*(len(self.limits)/2-0.5)]
        else:
            self.centerline_params = [self.params[0], self.scale + self.params[1]*(len(self.limits)/2-0.5)]
    ...
    def get_eqns(self):
        ...
        for i in range(len(self.limits)):
            params.append([self.params[0], self.scale + self.params[1]*i])
        return params

    def get_p_dist(self, xp, yp):
        x = Symbol('x')
        D_sq = (x-xp)**2 + (self.centerline_params[0]*x + self.centerline_params[1] - yp)**2
        D_sq_dot = D_sq.diff(x)
        x_0 = solve(Eq(D_sq_dot, 0), x)[0]
        min_dist = D_sq.subs({x:x_0})
        return min_dist**0.5

    def func_2d(self,X,a,b):
        Y = []
        ind = 0
        for i in range(len(self.limits)):
...
class Lane_optimizer_quad_new():
    ...
    def func_2d(self,X,a,b):
        ...
        for i in range(len(self.limits)):
            x = X[ind:int(self.limits[i])]
            Y.append(a * np.power(x,2) + b*i)
            ind = int(self.limits[i])
        y = [i for l in Y for i in l]
    
        return np.array(y)

class Lane_optimizer_quad():
    ...
    def update_vals(self, X, Y, limits, scale):
        ...
        for i in range(len(limits)):
            self.limits[i] = self.shape[i]
        self.params = self.fit_curve()
        #self.params_old = 0.95*np.array(self.params_old) + 0.05*np.array([self.params[0], self.params[1]])
        if len(self.limits)%2 == 0:
            self.centerline_params = [self.params[0], self.params[1], self.scale + self.params[2]*(len(self.limits)/2-0.5)]
        else:
            self.centerline_params = [self.params[0], self.params[1], self.scale + self.params[2]*(len(self.limits)/2-0.5)]
        #self.params[0] = self.params_old[0]
        #self.params[1] = self.params_old[1]
...
class MinimalSubscriber(Node):
    ...
    def __init__(self, display, font, clock):
        ...
        self.num_goal = 3
        self.L_linear = Lane_optimizer_linear()
        #self.L = Lane_optimizer_quad()
        #self.L = Lane_optimizer_quad_new()
    ...
    def integrand_quad_new(self, X, a):
        return (1 + (2*a*X)**2)**0.5

    def integrand_linear(self, X, a):
        return (1 + (a)**2)**0.5

    def integrand_3d(self, X, a, b, c):
        return (1 + (3*a*np.power(X,2) + 2*b*X + c)**2)**0.5
    ...
    def projection(self, pre_x, pre_y):
        ...
        print(np.arctan(2*self.L.centerline_params[0]*xdata_plot + self.L.centerline_params[1]))
        ...
        for i in range(len(limits)):
            ...
            self.ax1.plot(xdata_plot, ydata_plot, 'g--') 
        ...
        #print(s2-s1)
        self.ax1.set_xlim(-2, 95)
        self.ax2.set_xlim(-50, 95)
        self.ax1.set_ylim(-5, 50)
    ...
    def projection_linear(self):
        image_rgb = self.csm.sensor_data[1]
        array = np.frombuffer(image_rgb.raw_data, dtype=np.dtype("uint8"))
        array = np.reshape(array, (image_rgb.height, image_rgb.width, 4))
        array = array[:, :, :3]
        lane_pt, world_pt = self.csm.projection_new(num=5)
        img = array.copy()
        ...
        """for i in range(len(limits)):
            ...
            ind = int(limits[i])"""
        #print("New = ", Y)
        #print(X.shape, Y.shape, limits)
        self.L_linear.update_vals(X, Y, limits, min_y)
        params = self.L_linear.get_eqns()
        xdata_plot = np.linspace(0, 100, 200)
        #plt.plot(xdata1, ydata1)
        #plt.plot(xdata2, ydata2)
        #plt.plot(xdata3, ydata3)
        #d = self.L_linear.get_p_dist(0,0)
        x_pose = 0
        y_pose = -self.L_linear.centerline_params[1]
        cyy = self.L_linear.centerline_params[1]
        self.L_linear.centerline_params[1] = 0
        y_centerline = self.func_linear(xdata_plot, *self.L_linear.centerline_params)
        psi = -np.arctan2(y_centerline[1]-y_centerline[0], xdata_plot[1]-xdata_plot[0])
        #print(np.arctan(self.L_linear.centerline_params[0]))

        #for pts in world_pt:
        #    #print(pts)
        #    self.ax1.scatter(pts[0], -1*pts[1]-cyy, alpha = 0.25)#'orange')
        for i in range(len(limits)):
            params[i][1] = params[i][1] - cyy
            ydata_plot = self.func_linear(xdata_plot, *params[i])
            III = quad(self.integrand_linear, 0, 50, args=(params[i][0]))
            self.ax1.plot(xdata_plot, ydata_plot, 'r--')
        self.ax1.plot(xdata_plot, y_centerline, 'r--')

        """xcoord = np.linspace(0, 100, 200)
        print("centerline params = ", self.L_linear.centerline_params)

        arc_length = [quad(self.integrand_linear, 0, i, args=(self.L_linear.centerline_params[0]))[0] for i in xcoord]
        ycoord = self.func_linear(xcoord, *self.L_linear.centerline_params)
        psicoord = np.arctan2(np.diff(ycoord), np.diff(xcoord))
        #print(psicoord)
        ...
        if len(self.steps)>0:
            for i in range(self.num_goal):
                ...
                for xc in range(2, len(self.pre_x[prev:prev+self.steps[i]])):
                    ...
                    current_arc_length_diff = self.pre_x[prev+xc] - arc_length[max_idx-1]
                    centerline_x = xcoord[max_idx-1] + current_arc_length_diff*total_x_diff/total_arc_length_diff
                    centerline_y = self.func_linear(centerline_x, self.L_linear.centerline_params[0], self.L_linear.centerline_params[1])
                    slope_tangent = self.L_linear.centerline_params[0]
                    theta_perpendicular = np.arctan(slope_tangent) + np.pi/2
                    x_g = centerline_x + self.pre_y[prev+xc]*np.cos(theta_perpendicular)
                    y_g = centerline_y + self.pre_y[prev+xc]*np.sin(theta_perpendicular)
                    path_x_coor.append(x_g)
    ...
    def projection_quad_new(self):
        image_rgb = self.csm.sensor_data[1]
        array = np.frombuffer(image_rgb.raw_data, dtype=np.dtype("uint8"))
        array = np.reshape(array, (image_rgb.height, image_rgb.width, 4))
        array = array[:, :, :3]
        lane_pt, world_pt = self.csm.projection_new()
        img = array.copy()
        for points in lane_pt:
            for p in points:
                if p[0]>=0 and p[1]>=0:
        ...
        #d = self.L.get_p_dist(0,0)
        x_pose = 0
        y_pose = -self.L.centerline_params[1]
        cyy = self.L.centerline_params[1]
        self.L.centerline_params[1] = 0
        y_centerline = self.func_quad_new(xdata_plot, *self.L.centerline_params)
        psi = -np.arctan2(y_centerline[1]-y_centerline[0], xdata_plot[1]-xdata_plot[0])
        print(np.arctan(2*self.L.centerline_params[0]*xdata_plot))
        ...
        for i in range(len(limits)):
            params[i][1] = params[i][1] - cyy
            ydata_plot = self.func_quad_new(xdata_plot, *params[i])
            III = quad(self.integrand_quad_new, 0, 50, args=(params[i][0]))
            self.ax1.plot(xdata_plot, ydata_plot, 'g--')
        self.ax1.plot(xdata_plot, y_centerline, 'b--')
        ...
        arc_length = [quad(self.integrand_quad_new, 0, i, args=(self.L.centerline_params[0]))[0] for i in xcoord]
        ycoord = self.func_quad_new(xcoord, *self.L.centerline_params)
        psicoord = np.arctan2(np.diff(ycoord), np.diff(xcoord))
        print(psicoord)
        ...
        if len(self.steps)>0:
            for i in range(self.num_goal):
                ...
                for xc in range(2, len(self.pre_x[prev:prev+self.steps[i]])):
                    ...
                    current_arc_length_diff = self.pre_x[prev+xc] - arc_length[max_idx-1]
                    centerline_x = xcoord[max_idx-1] + current_arc_length_diff*total_x_diff/total_arc_length_diff
                    centerline_y = self.func_quad_new(centerline_x, self.L.centerline_params[0], self.L.centerline_params[1])
                    slope_tangent = 2*self.L.centerline_params[0]*np.array(centerline_x)
                    theta_perpendicular = np.arctan(slope_tangent) + np.pi/2
                    x_g = centerline_x + self.pre_y[prev+xc]*np.cos(theta_perpendicular)
                    y_g = centerline_y + self.pre_y[prev+xc]*np.sin(theta_perpendicular)
                    path_x_coor.append(x_g)
    ...
    def projection_quad(self):
        image_rgb = self.csm.sensor_data[1]
        array = np.frombuffer(image_rgb.raw_data, dtype=np.dtype("uint8"))
        array = np.reshape(array, (image_rgb.height, image_rgb.width, 4))
        array = array[:, :, :3]
        lane_pt, world_pt = self.csm.projection_new(100)
        img = array.copy()
        for points in lane_pt:
            for p in points:
                if p[0]>=0 and p[1]>=0:
    ...
    def projection_cubic(self):
        ...
        y_centerline = self.func_3d(xdata_plot, *self.L.centerline_params)
        #psi = -np.arctan2(y_centerline[1]-y_centerline[0], xdata_plot[1]-xdata_plot[0])
        ...
        s2 = time.time()
        #print("time = ", s1-s2)
        ycoord = self.func_3d(xcoord, *self.L.centerline_params)
        psicoord = np.arctan2(np.diff(ycoord), np.diff(xcoord))
        ...
        if len(self.steps)>0:
            for i in range(self.num_goal):
                ...
                for xc in range(2, len(self.pre_x[prev:prev+self.steps[i]])):
                    ...
                    current_arc_length_diff = self.pre_x[prev+xc] - arc_length[max_idx-1]
                    centerline_x = xcoord[max_idx-1] + current_arc_length_diff*total_x_diff/total_arc_length_diff
                    centerline_y = self.func_3d(centerline_x, self.L.centerline_params[0], self.L.centerline_params[1], self.L.centerline_params[2], self.L.centerline_params[3])
    ...
    def projection_cubic_new(self):
        image_rgb = self.csm.sensor_data[1]
        array = np.frombuffer(image_rgb.raw_data, dtype=np.dtype("uint8"))
        array = np.reshape(array, (image_rgb.height, image_rgb.width, 4))
        array = array[:, :, :3]
        lane_pt, world_pt = self.csm.projection_new(num=120)
        img = array.copy()
        for points in lane_pt:
            for p in points:
                if p[0]>=0 and p[1]>=0:
        ...
        cl_knots = []
        for l in ind_min:
            tck = interpolate.splrep(X_input[l][0], -1*X_input[l][1], s=5, k=3)
            uu=np.linspace(0,100,num=1000,endpoint=True)
            pp = PPoly.from_spline(tck)
            idx = 1
            tck[0][-1] = 100
            for coeff in pp.c.T:
                u=np.linspace(tck[0][idx-1],tck[0][idx],num=100,endpoint=True)-tck[0][idx-1]
                yp = coeff[0]*np.power(u,3) + coeff[1]*np.power(u,2) + coeff[2]*np.power(u,1) + coeff[3]
                if tck[0][idx]>tck[0][idx-1]:
                    psi = np.arctan2(np.diff(yp), np.diff(u+tck[0][idx-1]))
                    #print(u, yp, tck[0][idx-1],tck[0][idx])
                    #print("idx = ", psi*180/np.pi)
                plt.plot(u+tck[0][idx-1], yp, color='red')
                idx +=1
        s2 = time.time()
        #print(s2-s1)
        ...
        if len(self.steps)>0:
            for i in range(self.num_goal):
                if i == self.index:
                    ...
                    self.ax2.plot(self.pre_x[prev+2:prev+self.steps[i]], self.pre_y[prev+2:prev+self.steps[i]], alpha = 0.75, linewidth=2.0)#, color='green')#'orange')
                prev += self.steps[i]
        self.ax1.set_xlim(-2, 125)
        self.ax2.set_xlim(-50, 95)
        self.ax1.set_ylim(-5, 60)
        self.ax2.set_ylim(-5, 5)
        #plt.draw()
        plt.pause(0.0001)
        #plt.show()
