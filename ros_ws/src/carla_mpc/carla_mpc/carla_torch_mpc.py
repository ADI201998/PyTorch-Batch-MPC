...
pkg_path = str(Path(dir_name+str("/src/PyTorchMPC")))
try:
    print(pkg_path)
    sys.path.append(pkg_path)
except:
...
from carla_generative_torch import PyTorch_MPC
...
class MinimalSubscriber(Node):
    ...
    def __init__(self, display, font, clock):
        ...
        self.other_vehicles = [[-10, 1.75, 0,0,1000],
                                [15, -1.75, 0,0,1000],
                                [35, 1.75, 0,0,1000],
                                [65, -1.75, 0,0,1000],
                                [85, 1.75, 0,0,1000],
    ...
    def projection_cubic(self):
        ...
        if 1:
            for i in range(1):
                ...
                cent_x = []
                cent_y = []
                for xc in range(len(self.pre_x)):
                    max_idx = np.where((arc_length >= self.pre_x[xc]))[0]
                    if len(max_idx)<1:
                        continue
                    max_idx = max_idx[0]
        ...
        if len(self.steps)>0:
            for i in range(self.num_goal):
                if i == self.index:
                    self.ax2.plot(self.pre_x, self.pre_y, alpha = 1, linewidth=4.0, color='red')#'orange')
                    self.ax2.scatter(self.pre_x[prev+self.steps[i]-1], self.pre_y[prev+self.steps[i]-1], color='red')#'orange')
                else:
                    self.ax2.plot(self.pre_x, self.pre_y, alpha = 0.75, linewidth=2.0)#, color='green')#'orange')
    ...
    def projection_quadratic(self):
        ...
        all_lane_pix_pts = []
        s2= time.time()
        #print("Projection of lidar to img time = ", s2-s1)
        ###############################################
        ...
        if len(all_lane_pix)>0:
            for lane in all_lane_pix:
                ...
                lane_pix_pts.append(lp_pix_pt)   
        s2 = time.time()
        #print("Get ind lanes time = ", s2-s1)
        ###############################################
        ...
        cv2.waitKey(10)
        #s2 = time.time()
        #print("CV2 lane proj time = ", s2-s1)
        ###############################################
        ...
        X = np.array(X) - self.min_x
        s2 = time.time()
        #print("Get X Y conversion time = ", s2-s1)
        ###############################################
        ...
        """self.ax1.scatter(np.array(all_lane_pts)[:, 0], -1*np.array(all_lane_pts)[:, 1]-cyy)
        s2 = time.time()
        #print("Plot Centerline Eqns time = ", s2-s1)
        ###############################################"""
        ...
        if 1:
            for i in range(self.num_goal):
                ...
                cent_x = []
                cent_y = []
                for xc in range(2, len(self.pre_x)):
                ...
                if i == self.index:
                    ...
                    print(path_psi[:30])
                    omega = np.diff(np.array(path_psi))/self.dt
                    self.omega = 1*np.mean(omega[:10])
                else:
                    self.ax1.plot(path_x_coor, path_y_coor, alpha = 0.75, linewidth=2.0)#'orange')
        t2 = time.time()
        #print("Path conversion to func time = ", t2-t1)
        ###############################################
        ...
        prev = 0
        if 1:
            self.ax2.plot(self.xt, self.yt, linewidth=0.05, color='blue')
            self.ax2.plot(self.pre_x, self.pre_y, alpha = 1, linewidth=4.0, color='red')
            """for i in range(self.num_goal):
                if i == self.index:
                    self.ax2.plot(self.pre_x, self.pre_y, alpha = 1, linewidth=4.0, color='red')#'orange')
                    self.ax2.scatter(self.pre_x, self.pre_y, color='red')#'orange')
                else:
                    self.ax2.plot(self.pre_x, self.pre_y, alpha = 0.75, linewidth=2.0)#, color='green')#'orange')"""

        s2 = time.time()
        #print("Plot Paths time = ", s2-s1)
        ###############################################
    ...
    def timer_callback(self):
        ...
        if (self.obs[0][0] < 600 and self.setting == 2) or (self.obs[0][1] >= -8.0-0.7 and self.setting == 1) or (self.obs[0][0] < 1000 and self.setting == 0) or (self.obs[0][0] < 400 and self.NGSIM == True):#self.obs[0][1] >= -8.0-0.7:#self.obs[0][0] < 1000:#self.obs[0][1] >= -8.0-0.7:#self.obs[0][0] < 1000 or self.obs[0][1] >= -10:
            ...
            if 1:
                ...
                #t1 = time()
                dt = self.dt
                acc = np.clip(-3.0, (self.v-self.prev_vel)/dt, 3.0)
                x_init = self.obs[0][0]
                y_init = self.obs[0][1]
                vx_init = self.obs[0][2]
                vy_init = self.obs[0][3]
                ax_init = acc*np.cos(self.psi)
                ay_init = acc*np.sin(self.psi)
                psi_init = self.psi
                psidot_init = self.omega
                x_obs = []
                y_obs = []
                for i in range(len(self.other_vehicles)):
                    x_obs.append(self.other_vehicles[i][0])
                    y_obs.append(self.other_vehicles[i][1])
                while len(x_obs)<10:
                    x_obs.append(150)
                    y_obs.append(1.75)

                print("Input = ", x_init, vx_init, ax_init, y_init, vy_init, ay_init, psi_init, psidot_init)
                self.pre_x, self.pre_y, self.v, self.pre_psi, xt, yt = self.pm.generate_samples(x_init, vx_init, ax_init, y_init, vy_init, ay_init, psi_init, psidot_init, x_obs, y_obs)
                self.pre_x = self.pre_x.cpu().detach().numpy()
                self.pre_y = self.pre_y.cpu().detach().numpy()
                self.v = self.v.cpu().detach().numpy()
                self.pre_psi = self.pre_psi.cpu().detach().numpy()
                print(xt.shape, yt.shape)
                print(xt.shape, yt.shape)
                print(self.pre_y[:20])
                self.xt = xt.T.cpu().numpy()
                self.yt = yt.T.cpu().numpy()
                #self.ax2.plot(self.pre_x.T, self.pre_y.T, linewidth=5.0, color='red')
                #self.ax2.plot(xt.T.cpu().numpy(), yt.T.cpu().numpy(), linewidth=0.05, color='blue')
                ...
                t2 = time.time()
                #print("Simulate and centerline_conversioon time =  ", t2-t1)
                #print()
                #print()
                print()
                ...
                t2 = time.time()
                #print("Quadratic Projection time", t2-t1)
                #print()
                #print()
                #print()
