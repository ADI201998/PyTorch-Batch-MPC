...
class MinimalPublisher : public rclcpp::Node
{
	...
	MinimalPublisher(): Node("minimal_publisher"), count_(0)
    {
		...
		YAML::Node map = YAML::LoadFile("src/mpc_car_acado/config.yaml");
        
        string setting = map["setting"].as<string>();

        num_goal = map["configuration"][setting]["goal"].as<float>();
...
void MinimalPublisher :: get_ranks()
{
    ...
    for(int i = 0; i < num_goal; i++)
    {
        meta_cost(i, 1) = (batch_v.row(i) - v_cruise).matrix().lpNorm<2>(); // 5
        meta_cost(i, 2) = batch_optimal.row(i).matrix().lpNorm<2>();        // 6
        meta_cost(i, 3) = (batch_y.row(i) -  (0)).matrix().lpNorm<2>(); // 7
        meta_cost(i, 4) = (batch_v.row(i) - 24).matrix().lpNorm<2>();                       // 8
        meta_cost(i, 5) = -1;
        meta_cost(i, 6) = -1;
        meta_cost(i, 7) = -1;
        meta_cost(i, 8) = -1;
...
void MinimalPublisher :: timer_callback()
{
	...
	if(done)
	{
		...
		// cruise 30,70 ngsim data2


		//ngsim2 hsrl 0,50,5,1
		int index = 0;
		get_ranks();
		float min = 100000000;
		for(int i = 0; i < num_goal; i++)
		...
		for(int i = 0; i < num_goal; i++)
		{
			for(int j = 0; j < num; j++)
			{
				...
				pose.orientation.z = batch_theta(i, j);
...
void MinimalPublisher :: optimize(int info)
{
	...
	{	
		...
		for (int i = 0; i < N * (1 - first); i++)
		{
			acadoVariables.W[NY*NY*i + (NY+1)*0] = 7*1e3;	// a
			acadoVariables.W[NY*NY*i + (NY+1)*1] = 7*1e6;	// j
			acadoVariables.W[NY*NY*i + (NY+1)*2] = 3*2.8*1e4;
			acadoVariables.W[NY*NY*i + (NY+1)*3] = 3*2.8*1e4;
			acadoVariables.W[NY*NY*i + (NY+1)*4] = 3*2.8*1e4;
		...
		acadoVariables.WN[(NYN+1)*0] = 1e9;		// x terminal
		acadoVariables.WN[(NYN+1)*1] = 1e9;		// y terminal
